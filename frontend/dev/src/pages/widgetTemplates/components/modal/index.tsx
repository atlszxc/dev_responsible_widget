...
import AdditionOptions from '../additionOptions/additionOptions.tsx';
                ...
                <div className={Styles["pattern-modal__body"]}>
                    <div className={classNames(Styles["pattern-modal__general"], Styles["pattern-modal__options"])}>
                        ...
                        <div className={Styles["pattern-modal__general--top"]}>
                            <Input className="pattern-modal__pattern-name" style={{ flex: '0 1 50%'}} value={templateName} placeholder="Введите название шаблона" extraOptions={{ width: 'w-full' }} onChange={setTemplateName} />
                            <Select 
                                onChange={(e) => setAlgoritm(e)} defaultValue={template? template.alghoritm : null} 
                                placeholder='Выберите алгоритм распределения' 
                                isSearchable 
                                className={classNames(Styles['select'], Styles['flex-50'])} 
                                options={ALGHPRITMS} 
                            />
                        </div>
